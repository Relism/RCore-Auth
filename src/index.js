  ...
  if (hours1 == hours2 && minutes1 != minutes2) {
    ...
    if (subtraction > 60) {
      ...
      if (remainingHours == 1) {
        ...
        difference =
          "Fra " + remainingHours + " ore, " + remainingMinutes + " minuti";
      }
    } else {
...
client.on("message", (message) => {
  ...
  if (messaggio.startsWith("fermata") || messaggio.startsWith("Fermata")) {
    ...
    axios
      .get(
        "https://gttapi.relism.repl.co/?token=" +
          api_token +
          "&fermata=" +
          fermata
      )
      .then((resp) => {
        ...
        if (data != "No such stop found") {
          ...
          newLog(
            "+" + author + " has requested and acquired data for bus " + fermata
          );
        } else {
